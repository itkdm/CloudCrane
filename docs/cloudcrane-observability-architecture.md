...
`@cloudcrane/observability` 保留旧 `createLogger(service)` API，并通过 AsyncLocalStorage 自动注入上下文。日志基础字段为 `timestamp/level/service/environment/version/commitSha/region/event/message`；上下文按需包含 `requestId/traceId/spanId/runCorrelationId/userId/websiteId/workspaceId/sessionId/agentRunId/runnerId/connectionId/operation`，未提供的字段不会输出。
...
`audit_event` 是控制面唯一审计表，业务对象删除时相关外键使用 `SET NULL`，不会级联删除证据。状态为 `PENDING/RUNNING/SUCCESS/FAILED/TIMEOUT/CANCELLED/UNKNOWN`；跨 PostgreSQL 与 Docker/Runner 的操作应先提交 PENDING，远程执行后再写终态。连接中断且结果不确定时只能写 UNKNOWN，不能误记 FAILED 或自动重试。
...
Node 服务默认输出 JSON stdout，由 tmux 或 systemd/journald 管理；应用不自行创建无限增长的日志文件。PostgreSQL Compose 和 Workspace Docker 应使用 `json-file` 的大小/文件数限制。Nginx 模板显式定义 CloudCrane access/error 日志路径，主机通过 logrotate 或 journald 配置保留周期。

`deploy/systemd` 和 `deploy/alloy` 只提供可审查模板，不会自动修改 ECS，也不会替换当前 tmux 生产进程。集中采集器必须从主机私有配置注入 endpoint/凭据；仓库模板不包含真实 Secret，也不采集 Workspace 内容。
