...
本文记录 CloudCrane 默认验收拓扑与备用本地开发拓扑的启动关系，以及网络中断后的恢复方法。原阿里云 ECS 已于 2026-09-17 因服务器到期完全废弃；当前统一使用 SSH 别名 `xunmao-sg219` 的新加坡服务器。文档不保存数据库密码、SSH 私钥、模型凭据、Preview Token 或 PbootCMS 授权码。

## 服务器 SSH 连接

本项目后续部署和服务器排查使用本机 SSH 配置中的 `xunmao-sg219` 主机别名：

```powershell
ssh xunmao-sg219
```

已验证该命令可以连通新服务器 `ser645697687930`（公网 IPv4：`186.244.238.219`）。该服务器网络可以正常访问 GitHub 和 Docker Hub；基础设施与 CloudCrane 服务部署以该服务器为准。
