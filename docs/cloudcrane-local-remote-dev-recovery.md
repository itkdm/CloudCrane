...
本文记录 CloudCrane 默认验收拓扑与备用本地开发拓扑的启动关系，以及网络中断后的恢复方法。原阿里云 ECS 已于 2026-09-17 因服务器到期完全废弃；后续以 `xunmao-sg44` 对应的新服务器为部署候选目标，但该服务器尚未完成 CloudCrane 初始化。文档不保存数据库密码、SSH 私钥、模型凭据、Preview Token 或 PbootCMS 授权码。

## 服务器 SSH 连接

本项目后续部署和服务器排查使用本机 SSH 配置中的 `xunmao-sg44` 主机别名：

```powershell
ssh xunmao-sg44
```

已验证该命令可以连通新服务器 `ser786051264801`。当前该服务器只是可用的 SSH 目标，尚未安装 Docker、创建 `/opt/cloudcrane` 或运行 CloudCrane 服务；部署前需要先完成服务器初始化和服务栈部署。
