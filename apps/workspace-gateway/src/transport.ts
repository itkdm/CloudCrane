...
export function attachRunnerTransport(
  ...
  logger: ServiceLogger = createLogger('workspace-gateway.socket'),
) {
  ...
  server.on('connection', (socket: WebSocket, request: import('node:http').IncomingMessage) => {
    ...
    const connectionContext = {
      ...
      operation: 'workspace.runner.websocket',
    } as const;
    runWithLogContext(connectionContext, () => {
      logger.info({ event: 'ws.connected', runnerId }, 'runner websocket connected');
    });
    socket.on('message', async (raw) => {
      await runWithLogContext(
        { ...connectionContext, runnerId, operation: 'workspace.runner.message' },
        async () => {
          try {
            ...
            if (data.type === 'runner.register') {
              ...
              logger.info(
                { event: 'runner.connected', runnerId, outcome: 'succeeded' },
                'runner registered',
              );
              socket.send(
                JSON.stringify({
            ...
            logger.warn(
              {
                event: 'ws.protocol.error',
                runnerId,
                outcome: 'failed',
                ...serializeError(error),
              },
              'runner websocket protocol message rejected',
            );
    ...
    socket.on('close', () => {
      ...
      runWithLogContext(
        { ...connectionContext, runnerId, operation: 'workspace.runner.websocket' },
        () => logger.info({ event: 'ws.disconnected', runnerId }, 'runner websocket closed'),
      );
    });
  });
